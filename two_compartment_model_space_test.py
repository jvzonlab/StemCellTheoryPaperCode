...
import two_compartment_model_lib as model
...
sns.heatmap(S_expt, ax=ax_A[0], vmin = 0.03, vmax=0.41, mask=mask, cmap=cmap, cbar=False, xticklabels = states_lbl, yticklabels = states_lbl)
...
sns.heatmap(C_expt, ax=ax_A[1], vmin = 0.03, vmax=0.5, mask=mask, cmap=cmap, cbar=True, cbar_ax = ax_A_cb, cbar_kws ={'label':'Fraction'}, xticklabels = states_lbl, yticklabels = [])
...
for m in range(0,3):
    ...
    for n in range(0,50):
        ...
        for l in range(0,len(data['Lineage'])):
        
            L = data['Lineage'][l]
            # W = L.draw_lineage(t_sim,x0)  
            # x0 += W
            stats = L.count_divisions()
    ...
    if m==0:
        sns.heatmap(C, vmin = 0.03, vmax=0.5, ax=ax_B[m], mask=mask, cmap=cmap, cbar=False, xticklabels = states_lbl, yticklabels = states_lbl)
    elif m==1:
        sns.heatmap(C, vmin = 0.03, vmax=0.5, ax=ax_B[m], mask=mask, cmap=cmap, cbar=False, xticklabels = states_lbl, yticklabels = [])
    else:
        sns.heatmap(C, vmin = 0.03, vmax=0.5, ax=ax_B[m], mask=mask, cmap=cmap, cbar=True, cbar_ax = ax_B_cb, xticklabels = states_lbl, yticklabels = [])
...
ax_B[0].set_ylabel('S1', fontsize=10)
