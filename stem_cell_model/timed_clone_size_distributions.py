...
def get_clone_size_distribution(lineages: Lineages, min_time: float, max_time: float, interval: float,
                                              ) -> TimedCloneSizeDistribution:
    """Gets the normal clone size distribution of this lineage tree. Note that this clone size
    distribution will grow forever, as there is no way for cells to be removed from the simulation."""
    current_max_time = min_time + interval
    distributions = list()
    while current_max_time <= max_time:
        distribution = CloneSizeDistribution()
        for track in lineages.get_tracks():
            if track.exists_at_time(min_time) and track.compartment.get_compartment_at(min_time) == 0:
                distribution.add_clone_size(track.get_clone_size(current_max_time))
        distributions.append(distribution)
        current_max_time += interval
